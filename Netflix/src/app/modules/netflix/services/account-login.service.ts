...
export class AccountLoginService {
  ...
  public addDefault(){
    localStorage.setItem(this.defAccount.email, JSON.stringify(this.defAccount));
  }
  ...
  public addAccount(email:string, pass:string, users:IUser[]=[{user:'Main', img:'https://i.pinimg.com/236x/97/7f/e7/977fe798cf2c3a037e7aa9af6ce4b9d1.jpg'}]){
    ...
    //   {email,pass, users}
    // )
    localStorage.setItem(email, JSON.stringify({email, pass, users}));
  }

  public findAccount(email:string, pass:string):IAccount|undefined{
    const account:IAccount = JSON.parse('' + localStorage.getItem(email));
    console.log(account);
